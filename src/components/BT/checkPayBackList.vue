        ...
        <div class="page-loadmore mt5" style="overflow-x: hidden;">
          <div ref="wrapper" :style="{ height: wrapperHeight + 'px' }" style="overflow-y: auto;">
            <mt-loadmore
              ...
              <!--全部-->
              <ul class="f3 check-payback-list" v-for="item in payList" :key="item.id" v-if="check_type === ''"  ref="wrapper" >
                <li class="pa3">
                  <p v-cloak>{{item.back_month}}</p>
                  <p class="c9 mt2 fz14" v-cloak>还款 ¥{{Number(item.total_money).toFixed(2)                                                                          }}</p>
                </li>
                <!--内容-->
                <router-link tag="li"
                  ...
                  <div class="flex-space-between mt2">
                    ...
                    <p class="fz20 fw5" v-cloak>¥ {{Number(lis.back_money).toFixed(2)}}</p>
                  </div>
                </router-link>
              </ul>
              <!--待确认已入账-->
              <div ref="wrapper" :style="{ minHeight: ulHeight + 'px' }" v-if="check_type !== ''">
                <ul class="f3 check-payback-list" v-for="item in payList" :key="item.id"  >
                  <router-link tag="li"
                    ...
                    <div class="flex-space-between mt2">
                      ...
                      <p class="fz20" v-cloak>¥ {{Number(item.back_money).toFixed(2)}}</p>
                    </div>
                  </router-link>
                </ul>
              </div>
